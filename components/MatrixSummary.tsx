    ...
    <div className="flex flex-col gap-12 py-16 animate-fade-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 max-w-6xl mx-auto w-full">
        ...
        <div className="md:col-span-4 h-full">
          <SummaryCard title="Central Core" color="#FBBF24" subtitle="Comfort Zone">
        ...
        <div className="md:col-span-8">
          <SummaryCard title="Identity Path" color="#F87171" subtitle="Pillars of Destiny">
        ...
        <div className="md:col-span-12 grid grid-cols-1 md:grid-cols-2 gap-6">
          <SummaryCard title="Evolutionary Arcs" color="#10B981" subtitle="Milestones of Growth">
            <div className="space-y-3">
              <div className="flex justify-between items-center p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
                ...
                <span className="text-sm font-bold text-emerald-400">{points.L2.number} — {getArcana(points.L2.number).name}</span>
              </div>
          ...
          <SummaryCard title="Karmic Channel" color="#8B5CF6" subtitle="Past & Heritage">
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-center gap-6 py-4 rounded-2xl bg-accent-purple/5 border border-white/[0.04]">
                {[points.M.number, points.N.number, points.D.number].map((n, idx) => (
                  <div key={idx} className="flex flex-col items-center">
                    <span className="text-2xl font-black text-accent-purple">{n}</span>
        ...
        <div className="md:col-span-6 h-full">
          <SummaryCard title="Relationship Alchemy" color="#F43F5E" subtitle="Connections & Affection">
        ...
        <div className="md:col-span-6 h-full">
          <SummaryCard title="Prosperity Flow" color="#34D399" subtitle="Work & Abundance">
            <div className="space-y-2">
        ...
        <div className="md:col-span-6">
          <SummaryCard title="Destiny Purpose" color="#FACC15" subtitle="Your Mission">
            <div className="space-y-2">
        ...
        <div className="md:col-span-6">
          <SummaryCard title="Sahasrara Vibration" color="#A78BFA" subtitle="Crown Chakra">
            <div className="space-y-2">
...
function SummaryCard({ title, subtitle, color, children }: { title: string; subtitle?: string; color: string; children: React.ReactNode }) {
  return (
...
function TalentBox({ title, value, highlight }: { title: string; value: number; highlight?: boolean }) {
  ...
  return (
    <div className={`p-6 flex flex-col items-center gap-2 transition-colors ${highlight ? 'bg-accent-purple/10' : 'hover:bg-white/[0.02]'}`}>
      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{title}</span>
      <span className={`text-4xl font-black ${highlight ? 'text-accent-purple' : 'text-white'}`}>{value}</span>
...
function DataRow({ label, value }: { label: string; value: number }) {
  ...
  return (
    <div className="flex items-center justify-between gap-4 py-2.5 border-b border-white/[0.03] last:border-0 group transition-colors">
      ...
      <span className="text-xs font-bold text-accent-purple text-right group-hover:scale-105 transition-transform">
        {value} — {arcana.name}
      </span>
    </div>
  )
